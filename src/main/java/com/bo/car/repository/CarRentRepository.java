...
public interface CarRentRepository extends JpaRepository<CarRentEntity, Long>{
    ...
    @Query("SELECT cr FROM CarRentEntity cr LEFT JOIN cr.car c " +
           " WHERE cr.status = 2 AND DATE_FORMAT(CURDATE(), '%Y-%m-%d') BETWEEN cr.startDate AND cr.endDate" +
     ...
     */
    @Query("SELECT COUNT(cr) FROM CarRentEntity cr LEFT JOIN cr.car c " +
           " WHERE cr.member.id = :memberId AND cr.status = 2 AND DATE_FORMAT(CURDATE(), '%Y-%m-%d') BETWEEN cr.startDate AND cr.endDate")
     ...
     */
    @Query("SELECT cr FROM CarRentEntity cr LEFT JOIN cr.car c " +
           " WHERE cr.status = 2 AND DATE_FORMAT(CURDATE(), '%Y-%m-%d') > cr.endDate" +
           " ORDER BY cr.reqDate DESC")
     ...
     */
    @Query("SELECT COUNT(cr) FROM CarRentEntity cr LEFT JOIN cr.car c " +
           " WHERE cr.member.id = :memberId AND cr.status = 2 AND DATE_FORMAT(CURDATE(), '%Y-%m-%d') > cr.endDate")
